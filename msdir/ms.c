...
void convertXtoADs(double *input, double *output, int n, double newmax){
  int i;
  double maxelement = -1.;
  for( i = 0; i < n; ++i)
...
int ms(int argc, char *argv[], double ***probMatrix, int *n, int **countVector, int* densityPoints, double **ads)
{
  int i, j, k, howmany, segsites , afreq;
	char **list, **cmatrix(), **tbsparamstrs ;
	double **sfsMatrix;
	FILE *pf, *fopen() ;
	double probss, tmrca, ttot ;
	void seedit( const char * ) ;
	int getpars( int argc, char *argv[], int *howmany )  ;
	int gensam( char **list, double *probss, double *ptmrca, double *pttot ) ;
	int sfsMatrixCapacity = 1000, *sfsCount = NULL, *capacities = NULL;
	...
	int parseRes = getpars( argc, argv, &howmany) ;   /* results are stored in global variable, pars */
	   ...
	   /* if( segsites > 0 )    fprintf(pf,"positions: "); */
		...
		for( i=0; i<segsites; i++){
		  for(k=afreq=0; k<pars.cp.nsam; k++)
		    afreq += ( (list[k][i] == '1') ? 1: 0 );
		  sfsCount[afreq]++;
		  if(sfsCount[afreq] > capacities[afreq]){
		    capacities[afreq] += capacities[afreq];
		    sfsMatrix[afreq] = (double*)realloc(sfsMatrix[afreq], capacities[afreq]*sizeof(double));
		    assert(sfsMatrix[afreq] != NULL);
		  }
		  sfsMatrix[afreq][sfsCount[afreq]-1] = posit[i];
		}
		/* 	for(i=0;i<pars.cp.nsam; i++) { */
		/* 	  for(j = 0; j < sfsCount[i]; ++j){ */
		/* 	    fprintf(pf,"%f ", sfsMatrix[i][j]); */
		/* 	  } */
		/* 	  fprintf(pf, "\n"); */
		/* 	} */
		/* } */
    ...
    for(i=0;i<probMatDim; i++) {
      qsort(sfsMatrix[i], sfsCount[i], sizeof(double), cmpfunc);
    }
    ...
    double step = log(1.0/minval) /(denPoints);
    ...
    double *lads = (double*)calloc(denPoints+1, sizeof(double));

    convertXtoADs(xpoints, lads, denPoints + 1, 12.0);
...
int getpars(int argc, char *argv[], int *phowmany )
{
	int arg=0, i, j, sum , pop , argstart, npop , npop2, pop2 ;
	...
	while(strcmp(argv[arg], "-ms") != 0 && arg < argc-1){
	  arg++;
	}
...
/* adna */
	pars.cp.nsamin = atoi( argv[arg+1] );
...
/* adna */
	...
	if(strcmp(argv[arg], "-ms") != 0)
	  arg = 3 ;
	else
	  arg+=3;

	fprintf(stderr, "samples: %d, reps: %d\n", pars.cp.nsam, *phowmany);
...
int pickbmf(nsam, mfreq, ptree, tt )
  int nsam, mfreq;
	struct node *ptree;
	double tt;
{
