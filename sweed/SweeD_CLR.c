...
void createPROBS (int probGrid)
{
	...
	{	
		...
		//fprintf(stderr, "%d %f %f\n", i, alignment->gridADs[i], interval);
	}
	//assert(0);
...
void createPROBS_MS (int *probGrid, double** probMatrix, int probMatrixX, int probMatrixY)
{
  
  *probGrid = probMatrixX;
  int i, j, k, l;
  double prob;	
  ...
  for(i=0;i<alignment->maxn-alignment->minn+1;i++)
    {
      alignment->gridProbs[i] = calloc((alignment->sequences+maxx_offset-minx_offset+1), sizeof(t_sfs*));//(alignment->maxx-alignment->minx+1));
      assert(alignment->gridProbs[i]!=NULL);
  ...
  alignment->gridADs = calloc((*probGrid),sizeof(t_sfs));
  assert(alignment->gridADs!=NULL);	
  ...
  rvLUT = calloc((*probGrid), sizeof(double*));
  assert(rvLUT!=NULL);
