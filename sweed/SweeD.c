...
int main(int argc, char** argv)
{
  ...
  lastCheckpointTime = mainTime0;
	...
	int grid = 0, 
	  ...
	  counterRepls = 0,
	  noSeparator = 0,
	  monomorphic = 0, i, j, threads = 1, curSFSsize=10, sequences=-1, analyticalSFS=0, strictPolymorphic = 0, generateVCFsamplelist=0,
	...
	*chromList = NULL;
	...
	double **probsMatrix = NULL;
	double *ms_ads = NULL;
	int msSampleSize = 0;
	int *msSFScountVector = NULL;
	int denPoints = 0;

	simulation_mode = mssims = msselsims = 0;
	...
	if(generateVCFchromlist)
	  ...
	  }else
	  {
		...
		if(alignmentIndex < 2){
		  if( mssims )
		    ms(argc, argv, &probsMatrix, &msSampleSize, &msSFScountVector, &denPoints, &ms_ads);
		  else if( msselsims )
		    mssel(argc, argv, &probsMatrix, &msSampleSize, &msSFScountVector, &denPoints, &ms_ads);

		  fprintf(stderr, "SIMULATIONS ... DONE\n");
		  
		  if(alignment->maxn != msSampleSize - 1){
		    fprintf(stderr, "alignment->maxn: %d, BUT msSampleSize is: %d\n", alignment->maxn, msSampleSize);
		    assert(alignment -> maxn >  msSampleSize - 1);
		  }

		  printf("\n\n====================================================================\n\n");
		  for( j = 0; j < denPoints; j++)
		    {
		      printf("%f ", ms_ads[j]);
		    }
		  printf("\n");
		  
		  for(i = 1; i < msSampleSize-1; ++i)
		    {
		      printf("class %d: ", i);
		      for( j = 0; j < denPoints; j++)
			{
			  printf("%f ", probsMatrix[i][j]);
			}
		      printf("\n");
		    }
		}
			...
			if(onlySFS==0)
			  {
			    ...
			    if(simulation_mode == 1){
			      FILE *modelOut = NULL;
			      /* I have implemented the calculation of the SFS to start from 0
				 However, it is possible that the first and last elemetns of the SFS to be 0
			      */
			      alignment-> startSFS = 0;

			      for( i = 0; i < denPoints; ++i)
				alignment->gridADs[i] = ms_ads[i];
			      alignment->logAD0 = log(alignment->gridADs[0]);

			      for(i = 0 ; i < alignment->sequences; ++i){
				for(j = 0; j < denPoints; ++j){
				  alignment->gridProbs[0][i][j] = probsMatrix[i][j];
				}
			      }
			      alignment->interval = (denPoints-1) / (log(alignment -> gridADs[denPoints-1]) - alignment->logAD0);

			      if( statCounter == 0){
				statCounter = 1;
				char modelOutName[1024];
				sprintf(modelOutName, "SweeD_SimulationModel.%s", runName); 
				modelOut = fopen(modelOutName, "w");
				
				for(j = 0; j < denPoints; ++j)
				  fprintf(modelOut, "%.15f ", alignment->gridADs[j]);
				fprintf(modelOut, "\n");

				for(i = 0 ; i < alignment->sequences; ++i){
				  //fprintf(modelOut, "%d", i);
				  for(j = 0; j < denPoints; ++j){
				    fprintf(modelOut, " %.15f", probsMatrix[i][j]);
				  }
				  fprintf(modelOut, "\n");
				}
				fclose(modelOut);
	...
	for(i = 0; i < msSampleSize; ++i)
	  free(probsMatrix[i]);
	free(probsMatrix);

	free(ms_ads);
	
	free(msSFScountVector);
