...
t_sfs splint(int x, int n, int gridSz, double ad)
{
  ...
  if(ad<=alignment->gridADs[0])
    {
      double y = alignment->gridProbs[indexn][indexx][0];
      //fprintf(stderr, "ad: %f, x: %d, n: %d, y: %f\n", ad, indexx, indexn, y);
      //assert( y >= 0);
      return y;
    }

  lInd = (int)((log(ad)-alignment->logAD0)*alignment->interval);
  if(lInd >= gridSz -1 )
    {
      //fprintf(stderr, "lind: %d, ad %f, ad0: %f, interval: %f\n", lInd, log(ad), alignment->logAD0, alignment->interval);
      double y = alignment->gridProbs[indexn][indexx][(gridSz-1)];
      //double y = alignment->gridProbs[indexn][indexx][(gridSz-1)*2];
      // WHY IT'S *2
      //fprintf(stderr, "ad: %f, x: %d, indexx: %d, n: %d, y: %f, gridSz: %d\n", ad, x, indexx, indexn, y, gridSz);
      /* int i; */
      /* for(i = 0; i < gridSz; ++i) */
      /* 	fprintf(stderr, "%d %f\n", i, alignment->gridADs[i]); */
      /* assert(y >= 0); */
      return y;
    }
  if (lInd<0)
  ...
  gridADDif = alignment->gridADs[rInd]-alignment->gridADs[lInd];
  ...
  if( y < 0){
    fprintf(stderr, "ad: %.10f\n");
    fprintf(stderr, "lind: %d, rind: %d\n", lInd, rInd);
    fprintf(stderr, "grid_left: %f\n", alignment->gridADs[lInd]);
    fprintf(stderr, "grid_right: %f\n", alignment->gridADs[rInd]);
	  ...
	  badFlag = 0;
		...
		if(alignment->positionsInd[i]!=-1)
		{
			...
			if(ad<12.0)
			{
				...
				if(ad < alignment->gridADs[0])
				  {
					...
					if( pr < 0){
					  fprintf(stderr, "n:%d x:%d pr:%f\n", n-alignment->minn, x - alignment->startSFS, pr);
					  assert(pr >= 0);
					}
				  }
				else
				  {
				    ...
				    if(pr < 0){
				      fprintf(stderr, "* ad: %f, n:%d x:%d pr:%f\n", ad, n-alignment->minn, x - alignment->startSFS, pr);
				      assert(pr >= 0);
				    }
				...
				if( pr < 0){
				  fprintf(stderr, "probability: %f, x: %d, n: %d, ad: %f\n", pr, x, n, ad);
				  assert(pr>=0.0);
				}
